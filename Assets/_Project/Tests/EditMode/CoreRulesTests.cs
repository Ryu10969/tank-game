...
namespace TankGame.Tests.EditMode
{
    public sealed class CoreRulesTests
    {
        ...
        [Test] public void ExactlyTwoReflectionsAreAllowed()
        {
            var rules = new ProjectileRules(2);
            Assert.That(rules.HitWall(), Is.True); Assert.That(rules.HitWall(), Is.True);
            Assert.That(rules.Reflections, Is.EqualTo(2)); Assert.That(rules.Alive, Is.True);
